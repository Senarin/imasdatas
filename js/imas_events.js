...
showGachaRemainingSL : function(){
 ...
 showRemainingTD : function(){
  ...
  r.onreadystatechange = function(){
   if(r.readyState == 4){
    if(r.status == 200){
     ...
     if($("#theater_elist_j td[data-event-id='"+eid+"'][data-event-id!='-1']").length != 0){var $eTitle = document.createTextNode($("#theater_elist_j td[data-event-id='"+eid+"'][data-event-id!='-1']").filter("[data-event-id='"+eid+"']").html().replace("<br>","\n").replace("&amp;","&"));}
  ...
  r.open("GET","https://api.matsurihi.me/mltd/v1/events?at="+dt);
  r.send();
 },
  ...
  r.onreadystatechange = function(){
   if(r.readyState == 4){
    if(r.status == 200){
     ...
     if($("#theater_elist_k td[data-event-id='"+eid+"'][data-event-id!='-1']").length != 0){var $eTitle = document.createTextNode($("#theater_elist_k td[data-event-id='"+eid+"'][data-event-id!='-1']").last().html().replace("<br>","\n").replace("&amp;","&"));}
     else{var $eTitle = document.createTextNode("개최 중인 이벤트가 없습니다.");}
...
var getSLGameClientInfo = {
 ...
 getContent : function(){
  ...
  r.open("GET","https://api.matsurihi.me/cgss/v1/version/latest");
  r.send();
 }
};
...
var getTDGameClientInfo = {
 ...
 getContent : function(){
  ...
  r.open("GET","https://api.matsurihi.me/mltd/v1/version/latest");
  r.send();
 }
};
...
var getTDEventRanksInfo = {
 ...
 retrieveData : function(eid){
  if(eid <= 0){alert("아직 개최되지 않은 이벤트입니다.");return;}
  if(eid < 33){alert("이벤트 포인트 랭킹 정보는 2018년 6월 개최된 시어터(PSTheater) 이벤트부터 확인 가능합니다.\n그 이전의 정보는 다음 웹사이트를 참고하시기 바랍니다:\n - https://si.ster.li/events/");}
   ...
   t.open("GET","https://api.matsurihi.me/mltd/v1/events/"+eid+"?prettyPrint=false");
   t.send();
  }
 }
};